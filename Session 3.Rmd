...
Practice importing datasets by going to [https://github.com/menawhalen/R_short_course_NCSBN/tree/main/data](https://github.com/menawhalen/R_short_course_NCSBN/tree/main/data) and download both dataset we have been using `CIACountries` and `flights`.
...
In many real-world datasets, information is stored in multiple tables. For example: - The `nycflights13` dataset contains flight information, airlines, airports, and planes all in separate tables. - The CTA ridership and stops datasets contain different types of information about Chicago train stations.
...
**Warm-Up:** \> If one dataset lists `station_id` and `rides` and another lists `station_id` and `line`, how could we find which line has the most rides?
...
We’ll use two real datasets: `CTA_ridership.csv` and `CTA_stops_long.csv`. We will also introduce the `janitor` package which features a function called `clean_names()` that changes all the variable names to be lowercase and separated with '_'.

```{r}
library(janitor)

rides <- read_csv("data/CTA_ridership.csv") %>%
  mutate(station_id = as.character(station_id))

stops <- read_csv("data/CTA_stops_long.csv") %>%
  clean_names() %>%
