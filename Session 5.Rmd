...
This is the most useful for modeling. Let's make a linear model.
Using broom::tidy to present the model output in "tidy" way.

```{r lm}
penguins %>% 
  group_by(species)  %>% 
  nest() %>% 
  mutate(model = map(data, ~ lm(flipper_length_mm ~ bill_length_mm, data = .))) %>%
