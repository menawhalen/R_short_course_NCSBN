...
description: > 
  Learning about the basics of the Tidyverse and data wrangling functions for one data frame.
---
...
4. Pipelining (`%>%` or `|>` Operator):

One of the defining features of dplyr is its compatibility with the pipe operator `%>%` or `|>`. This allows you to chain together multiple operations, improving code readability and making it easier to follow the flow of transformations.
...
Note: If you think typing the pipe is too cumbersome, use the shortcut Ctrl-Shift-m (Windows/Linux) or Cmd-Shift-m (Mac).
...
Let's see what happens when we use filter and select together.
...
If you encounter any rows with missing data, you can simply remove these rows by using na.rm = TRUE (or na.rm = T)

```{r summarize_cia_roadways}
my_CIACountries <-
  CIACountries %>% 
  mutate(high_educ = educ > 4.5) %>% 
  group_by(high_educ) %>% 
  summarise(mn_area = mean(roadways))
...
my_CIACountries <-
  ...
  mutate(high_educ = educ > 4.5) %>% 
  group_by(high_educ) %>% 
  summarise(mn_area = mean(roadways,na.rm=TRUE))
...
Explore CIA Countries dataset and answer the following questions.

1. How many variable names in the dataset contain the letter "p"?

2. How many countries begin with a vowel?

3. How many countries in the dataset produce oil?

4. Create a column for the roadways by area.

5. What are the 5 countries with the least densely populated roadways?

6. What is the average GDP separated by net users <15%, 15-60%, >60%?

7. What is the variance of population density by net users?

```{r explore_ex, exercise = TRUE}
CIACountries
```

## Creating Tables

If you are creating a pdf output for a report using markdown or LaTeX, the package `xtable` takes any dataset that you have created and turns it into a nice format that you can manipulate as you see fit! Please note, this section is best used in the R Window, not in the html interactive file.
...
We can also add a caption and label to the table.

```{r xtable_caption}
data %>%
  xtable(caption="This is the description for this data table.",
         label = "tbl1")
```

### Alignment

We can also change the alignment of the columns.
...
We can also change the number of significant digits.
...
By default, xtable creates a column for the row number.  To remove, simply another line for print and add `include_rownames = FALSE`.
